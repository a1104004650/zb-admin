...
export default {
  ...
  actions: {
    ...
    async openedLoad ({ state, commit, dispatch }) {
      // store 赋值
      const value = await dispatch('storeModules/db/get', {
        dbName: 'sys',
        path: 'page.opened',
    ...
    async close ({ state, commit, dispatch }, { tagName }) {
      ...
      // 决定最后停留的页面
      if (isCurrent) {
        const { name = 'portal', params = {}, query = {} } = newPage
    ...
    async closeLeft ({ state, commit, dispatch }, { pageSelect } = {}) {
      ...
      if (currentIndex > 0) {
        ...
        for (let i = state.opened.length - 1; i >= 0; i--) {
          if (state.opened[i].name === 'portal' || i >= currentIndex) {
            continue
          }
    ...
    async closeAll ({ state, commit, dispatch }) {
      ...
      if (router.app.$route.name !== 'portal') {
