...
export function pathInit ({
  ...
  const userInfo = util.cookies.get('userInfo') || {};
  const uuid = userInfo.userName || 'ghost-uuid';
  const currentPath = `${dbName}.${user ?  `${uuid}` : 'public'}.${path} `
  console.log(`#pathInit currentPath=${currentPath}`)
 ...
 * @param {Object} payload dbName {String} 数据库名称
 * @param {Object} payload path {String} 存储路径 可以理解为KEY
...
export function dbSet ({
  ...
  console.log(`#dbSet dbName=${dbName}; path=${path}; value=${value}; user=${user}`)
  db.set(pathInit({
    dbName,
    path,
    user
...
export function dbGet ({
  ...
  console.log(`#dbGet dbName=${dbName}; path=${path}; defaultValue=${defaultValue}; user=${user}`)
